...
-- Query to find the average customer rating and driver rating for each unique pickup location.
select Pickup_Location, ROUND(avg(customer_rating),2) AS avg_customer_rating, ROUND(avg(Driver_Ratings),2) AS avg_driver_ratings
from bookings
...
-- Bookings Per Day
select Date, dayname(Date) as dayname,count(*) as Total_bookings_per_day
from Bookings
group by Date
order by Total_bookings_per_day Desc;
